...
class RequestSite(object):
    ...
    def __init__(self, urls, ssl, tor):
        self.urls = urls
        self.tor = tor
        self.ssl = ssl
        self.my_user_agent = {
        ...
        self.tor_proxy = {'http': 'socks5h://127.0.0.1:9050',
    ...
    def request_site(self):
        ...
        site_request = self.session.get(
            self.urls, headers=self.my_user_agent, verify=self.ssl)
        return site_request
...
def main():
    ...
    for url in args.urls:
        ...
        table_format([site_requested.urls, 'Response'],
                     site_requested.return_site_status())

        if args.headers:
            table_format([site_requested.urls, 'Headers'],
                         site_requested.return_site_headers())
        if args.cookies:
            table_format([site_requested.urls, 'Cookies'],
